...
procedure TCustomSocket.SendStr(const Value: RawByteString);
begin
...
procedure TSOCKS5Socket.OnReceive;
  ...
  begin
    ...
    begin
      buf[3] := 3;
      buf[4] := len - 1;
      Move(PAnsiChar(Host)^, buf[5], len - 1);
    end else
    begin
      buf[3] := 1;
        ...
        begin
          ...
          begin
            ...
            begin
              ...
              FSrcBuf[0] := 1;
              FSrcBuf[1] := UserLen;
              Move(PAnsiChar(ProxyUser)^, Ptr(LongWord(@FSrcBuf) + 2)^, UserLen);
              FSrcBuf[UserLen + 2] := PassLen;
              Move(PAnsiChar(ProxyPass)^, Ptr(LongWord(@FSrcBuf) + 3 + UserLen)^, UserLen);
...
begin
  if not ProxyReady then begin
    ...
    begin
      ...
      if FLen = 0 then
        if FCurLen > 3 then
          if Copy(PAnsiChar(@FBuf), FCurLen - 3, 4) = #13#10#13#10 then
          begin
            List := TStringList.Create;
            List.Text := PAnsiChar(@FBuf);
            ...
            FCurLen := 0;
            {$IFDEF PARSE}
            LogText('http.txt', 'Received (proxy): '#13#10 + PAnsiChar(@FBuf));
            {$ENDIF}
          end;
